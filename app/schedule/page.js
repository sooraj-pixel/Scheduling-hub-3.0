...
// No longer need useRouter for internal toggling
// import { useRouter } from 'next/navigation'

// Import the MasterScheduleCalendarPage component
import MasterScheduleCalendarPage from '../master-schedule-calendar/page'; // Correct path relative to app/schedule/page.js
...
const Schedules = () => {
    ...
    const [filteredByTermSchedule, setFilteredByTermSchedule] = useState([]);
    ...
    }, [searchTerm, filteredByTermSchedule, scheduleInfo])

    const [visibleColumns, setVisibleColumns] = useState({});
    const [selectedEntry, setSelectedEntry] = useState(null);
    const [showForm, setShowForm] = useState(false);

    // ADDED: State to control calendar visibility
    ...
    }, [searchTerm, scheduleInfo])

    useEffect(() => {
        if (selectedTerm === "All Terms") {
            setFilteredByTermSchedule(scheduleInfo);
        } else {
            let updatedSchedule = scheduleInfo.filter(item =>
                Object.values(item).some(value =>
                    value && value.toString().toLowerCase().includes(selectedTerm.toLowerCase())
                )
            );
    ...
    const saveVisibility = () => {
        localStorage.setItem("schedule_visibleCols", JSON.stringify(visibleColumns));
    }

    // MODIFIED: handleViewCalendar now just toggles local state
    const handleViewCalendar = () => {
        setShowCalendar(true); // Show the calendar
    };
    // ADDED: Function to go back to schedule table
    const handleBackToSchedule = () => {
        setShowCalendar(false); // Hide the calendar, show the table
    };
    ...
    return (
        <Layout>
            <Section title={"Master Schedule"}>
                <div>
                    {/* Common Heading and Schedule dropdown (always visible) */}
                    ...
                    {/* Conditionally Render: Schedule Table vs. Calendar */}
                    {showCalendar ? (
                        // Display Calendar Page
                        <>
                            <div className='flex justify-end mb-4'>
                                <button className="btn-secondary" onClick={handleBackToSchedule}>
                                    &larr; Back to Schedule Table
                                </button>
                            </div>
                            {/* PASSSING selectedTerm AND terms AS PROPS TO MasterScheduleCalendarPage */}
                            <MasterScheduleCalendarPage selectedTerm={selectedTerm} terms={terms} />
                        </>
                    ) : (
                        // Display Master Schedule Table
                        <>
                            {/* Search, Download Excel, Add Entry, and View Schedule Calendar buttons */}
                            <div className="flex-between my-5">
                                ...
                                <div className='flex gap-3'>
                                    ...
                                    <button className="btn-primary flex-center" onClick={handleViewCalendar}>
                                        <img src="./svg/calendar.svg" alt="calendar icon" className="w-5 h-5 mr-1" />
