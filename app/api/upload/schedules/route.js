...
const tableName = "schedules"; // ✅ Changed from "instructors" to "schedules"
...
export const POST = async (req) => {
  ...
  try {
    ...
    const headers = raw[0].map(h =>
      h.toString().trim().replace(/[\s.&]/g, "_").toLowerCase()
    );
    ...
    jsonData.forEach(row => {
      Object.keys(row).forEach(key => {
        if (typeof row[key] === "number" && /(date)/i.test(key)) {
          row[key] = new Date((row[key] - 25569) * 86400000).toISOString().split("T")[0];
        } else if (typeof row[key] === "number" && /(time)/i.test(key)) {
          const timeStr = new Date((row[key] - 25569) * 86400000).toISOString().split("T")[1];
          row[key] = timeStr?.split(":" ).slice(0, 2).join(":" );
        }
      });
    });
    ...
    for (const row of jsonData) {
      ...
      const insertQuery = `
        INSERT INTO ${tableName} (${columns.join(', ')}, schedule_term)
        VALUES (${values}, '${fileName}')
      `;
      await db.query(insertQuery);
    }

    console.log(`✅ Uploaded ${jsonData.length} rows to ${tableName} as term '${fileName}'`);
