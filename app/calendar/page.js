...
import { Sun, Moon } from 'lucide-react'; // Import Sun and Moon icons for the toggle
...
export default function CalendarPage() {
  ...
  // Active tab in the event modal: 'event', 'reminder', 'share'
  ...
  const handleEventDrop = useCallback(
    ({ event, start, end, isAllDay }) => {
      ...
      setActiveTab('event'); // Ensure event tab is active
    },
    []
  );
  ...
  const handleSubmitEvent = useCallback(
    async (e) => {
      ...
      if (!tempEventTitle || !selectedDateForEvent || !tempEventFrom || !tempEventTo || !tempEventRecipientEmail) {
        alert('Please fill in all required fields: Title, Date, From, To, and Event Recipient Email.');
        return;
      }

      // Time validation for current day events
      ...
      if (isToday && eventStartTime.isBefore(now)) {
        ...
        setTimeValidationError(''); // Clear error if validation passes or not today's event
      }


      let finalLocation = '';
      if (tempLocationType === 'In Person') {
        if (!tempInPersonLocation) {
          alert('Please provide an In Person Location.');
          return;
        }
        finalLocation = `In Person: ${tempInPersonLocation}`;
      } else if (tempLocationType === 'Online') {
        if (!tempOnlineMeetingLink) {
      ...
      try {
        ...
        // Handle event sharing if emails are provided
        if (tempShareRecipientEmails && eventId) {
          ...
          if (shareEmailsArray.length > 0) {
            ...
            if (!shareResponse.ok) {
              ...
              alert('Event saved, but failed to share event. Please check console for details.');
            } else {
    ...
    [tempEventTitle, selectedDateForEvent, tempEventFrom, tempEventTo, selectedCalendar, tempLocationType, tempInPersonLocation, tempOnlineMeetingLink, tempEventColor, tempEventRecipientEmail, tempEnableEventReminder, tempShareRecipientEmails, editingEvent, fetchMyTodoEventsFromDb, resetModalForm]
  );
      ...
      <div className="flex-1 flex flex-col p-8 overflow-hidden">
        <header className="flex justify-between items-center bg-[#36454F] p-5 rounded-xl shadow-lg mb-8 border-b border-gray-100">
          ...
          <div className="flex items-center space-x-4">
            ...
            {/* Dark Mode Toggle Button (Functionality removed for now) */}
            <button
      ...
      {showEventModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto"> {/* Added max-h and overflow-y-auto */}
            ...
            {/* Tabs for Event Details, Reminders, and Sharing */}
            <div className="flex border-b border-gray-200 mb-4">
              <button
                className={`flex-1 py-3 text-lg font-medium ${activeTab === 'event' ? 'border-b-2 border-indigo-600 text-indigo-600' : 'text-gray-500 hover:text-gray-700'}`}
                onClick={() => setActiveTab('event')}
              >
                Event Details
              </button>
              <button
                className={`flex-1 py-3 text-lg font-medium ${activeTab === 'reminder' ? 'border-b-2 border-indigo-600 text-indigo-600' : 'text-gray-500 hover:text-gray-700'}`}
                onClick={() => setActiveTab('reminder')}
              >
                Reminders
              </button>
              <button
                className={`flex-1 py-3 text-lg font-medium ${activeTab === 'share' ? 'border-b-2 border-indigo-600 text-indigo-600' : 'text-gray-500 hover:text-gray-700'}`}
